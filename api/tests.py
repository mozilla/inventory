...
class SystemApi(TestCase):
    ...
    def test_get_system_not_found_by_id(self):
        resp = self.client.get('/api/system/-1/')
    ...
    def test_get_system_by_hostname(self):
        resp = self.client.get('/api/system/asfdasfasfasdfasfasdfsadf/')
        self.assertEqual(404, resp.status_code)
        resp = self.client.get('/api/system/fake-hostname2/')
        self.assertEqual(200, resp.status_code)

    def test_create_system(self):
        resp = self.client.post('/api/system/%s/' % self.new_hostname)
        self.assertEqual(201, resp.status_code)
        resp = self.client.get('/api/system/%i/' % 3)
        self.assertEqual(200, resp.status_code)
        resp = self.client.get('/api/system/%s/' % self.new_hostname)
        self.assertEqual(200, resp.status_code)
    ...
    def test_update_system(self):
        resp = self.client.post('/api/system/%s/' % self.new_hostname)
        self.assertEqual(201, resp.status_code)
        resp = self.client.put('/api/system/%i/' % (self.new_host_id), {'hostname':'updated_hostname'})
        self.assertEqual(200, resp.status_code)
        resp = self.client.get('/api/system/%i/' % (self.new_host_id))
        self.assertEqual(200, resp.status_code)
    ...
    def test_delete_system(self):
        resp = self.client.post('/api/system/%s/' % self.new_hostname)
        self.assertEqual(201, resp.status_code)
        resp = self.client.delete('/api/system/%i/' % (4))
        self.assertEqual(200, resp.status_code)
        resp = self.client.get('/api/system/%i/' % (self.new_host_id))
        self.assertEqual(404, resp.status_code)
        resp = self.client.get('/api/system/%s/' % self.new_hostname)
    ...
    def test_key_value_api(self):
        resp = self.client.get('/api/keyvalue/?keystore=fake-hostname2')
        print resp.content
...
class DHCPApi(TestCase):
    ...
    def test_get_single_scope(self):
        resp = self.client.get('/api/keyvalue/?key_type=dhcp_scopes')
    ...
    def test_get_system_by_scope(self):
        resp = self.client.get('/api/keyvalue/?key_type=system_by_scope&scope=phx-vlan73')
    ...
    def test_get_adapters_by_system(self):
        resp = self.client.get('/api/keyvalue/?key_type=adapters_by_system&system=fake-hostname2')
        system_list = json.loads(resp.content)
        #print system_list
    def test_delete_network_adapter(self):
        resp = self.client.delete('/api/keyvalue/1/', {'system_hostname':'fake-hostname2', 'adapter_number':'0', 'key_type':'delete_network_adapter'})
