...
class SystemApi(TestCase):
    ...
    def test_get_system_not_found_by_id(self):
        resp = self.client.get('/api/v2/system/-1/')
    ...
    def test_get_system_by_hostname(self):
        resp = self.client.get('/api/v2/system/asfdasfasfasdfasfasdfsadf/')
        self.assertEqual(404, resp.status_code)
        resp = self.client.get('/api/v2/system/fake-hostname2/')
        self.assertEqual(200, resp.status_code)

    def test_create_system(self):
        resp = self.client.post('/api/v2/system/%s/' % self.new_hostname)
        self.assertEqual(201, resp.status_code)
        resp = self.client.get('/api/v2/system/%i/' % self.new_host_id)
        self.assertEqual(200, resp.status_code)
        resp = self.client.get('/api/v2/system/%s/' % self.new_hostname)
        self.assertEqual(200, resp.status_code)
    ...
    def test_update_system(self):
        resp = self.client.post('/api/v2/system/%s/' % self.new_hostname)
        self.assertEqual(201, resp.status_code)
        resp = self.client.put('/api/v2/system/%i/' % (5), {'hostname':'updated_hostname'})
        self.assertEqual(200, resp.status_code)
        resp = self.client.get('/api/v2/system/%i/' % (5))
        self.assertEqual(200, resp.status_code)
    ...
    def test_delete_system(self):
        resp = self.client.post('/api/v2/system/%s/' % self.new_hostname)
        self.assertEqual(201, resp.status_code)
        resp = self.client.delete('/api/v2/system/%i/' % (4))
        self.assertEqual(200, resp.status_code)
        resp = self.client.get('/api/v2/system/%i/' % (4))
        self.assertEqual(404, resp.status_code)
        resp = self.client.get('/api/v2/system/%s/' % self.new_hostname)
        self.assertEqual(404, resp.status_code)
    ...
    def test_key_value_api(self):
        resp = self.client.get('/api/v2/keyvalue/?keystore=fake-hostname2')
        ...
        resp = self.client.put('/api/v2/keyvalue/5/', {'key':'nic.0.ipv4_address.0', 'value':'14.14.14.14', 'system_id':'1'})

        resp = self.client.get('/api/v2/keyvalue/?keystore=fake-hostname2')
        self.assertEqual(json.loads(resp.content)['host:fake-hostname1:nic.0.ipv4_address.0'], '10.99.32.3')

        resp = self.client.get('/api/v2/keyvalue/?key=cluster_owner')
        self.assertEqual(json.loads(resp.content)['truth:test:cluster_owner'], 'The Cluster Owner')

        resp = self.client.get('/api/v2/keyvalue/?value=10.99.32.3')
        self.assertEqual(json.loads(resp.content)['host:fake-hostname1:nic.0.ipv4_address.0'], '10.99.32.3')

    def test_search_by_asset_tag(self):
        resp = self.client.get('/api/v2/system/3/', {'search':True, 'asset_tag':'65432'})
    ...
    def test_search_by_serial(self):
        resp = self.client.get('/api/v2/system/3/', {'search':True, 'serial':'39993'})
        self.assertEqual(resp.status_code, 200)
    ...
    def test_search_by_serial_and_asset_tag_not_found(self):
        resp = self.client.get('/api/v2/system/3/', {'search':True, 'serial':'39993', 'asset_tag':'99999asdf'})
        self.assertEqual(resp.status_code, 404)

    def test_search_by_system_rack(self):
        resp = self.client.get('/api/v2/system/3/', {'search':True, 'system_rack_id':'1'})
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(json.loads(resp.content)[0]['hostname'], 'fake-hostname1')

    def test_search_by_system_rack_and_rack_order(self):
        resp = self.client.get('/api/v2/system/3/', {'search':True, 'system_rack_id':'1', 'rack_order':'1.00'})
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(json.loads(resp.content)[0]['hostname'], 'fake-hostname1')

    def test_search_by_system_rack_and_rack_order_not_found(self):
        resp = self.client.get('/api/v2/system/3/', {'search':True, 'system_rack_id':'1', 'rack_order':'2.00'})
        self.assertEqual(resp.status_code, 404)
        
    def test_search_by_system_rack_and_serial(self):
        resp = self.client.get('/api/v2/system/3/', {'search':True, 'system_rack_id':'1', 'serial':'39993'})
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(json.loads(resp.content)[0]['hostname'], 'fake-hostname1')

    def test_search_by_system_switch_ports(self):
        resp = self.client.get('/api/v2/system/3/', {'search':True, 'switch_ports':'101.02'})
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(json.loads(resp.content)[0]['hostname'], 'fake-hostname1')

    def test_search_by_system_switch_ports_not_found(self):
        resp = self.client.get('/api/v2/system/3/', {'search':True, 'switch_ports':'shouldnteverhavethisasaswitchport101.02'})
        self.assertEqual(resp.status_code, 404)

    def test_search_by_system_rack_and_serial_not_found(self):
        resp = self.client.get('/api/v2/system/3/', {'search':True, 'system_rack_id':'1', 'serial':'39993asdf'})
        self.assertEqual(resp.status_code, 404)
...
class DHCPApi(TestCase):
    ...
    def test_get_single_scope(self):
        resp = self.client.get('/api/v2/keyvalue/?key_type=dhcp_scopes')
    ...
    def test_get_system_by_scope(self):
        resp = self.client.get('/api/v2/keyvalue/?key_type=system_by_scope&scope=phx-vlan73')
    ...
    def test_get_adapters_by_system(self):
        resp = self.client.get('/api/v2/keyvalue/?key_type=adapters_by_system&system=fake-hostname2')
        system_list = json.loads(resp.content)
        #print system_list
    def test_delete_network_adapter(self):
        resp = self.client.delete('/api/v2/keyvalue/1/', {'system_hostname':'fake-hostname2', 'adapter_number':'0', 'key_type':'delete_network_adapter'})
...
class TestReverseDNSApi(TestCase):
    ...
    def test_get_single_reverse_zone_names_with_descriptions(self):
        resp = self.client.get('/api/v2/reverse_dns/1/get_reverse_dns_zones_with_names/')
        self.assertEqual(resp.status_code, 200)
        scope_list = json.loads(resp.content)
    ...
    def test_get_system_by_reverse_dns_zone(self):
        resp = self.client.get('/api/v2/keyvalue/?key_type=system_by_reverse_dns_zone&zone=phx1-32.8.10.in-addr.arpa')
        self.assertEqual(resp.status_code, 200)
        system_list = json.loads(resp.content)
...
class KeyValueApi(TestCase):
    ...
    def test_get_adapters_by_system(self):
        resp = self.client.get('/api/v2/keyvalue/3/', {'key_type':'adapters_by_system','system':'fake-hostname2'})
        #print resp.content

    def test_keyvalue_set_invalid_ip(self):
        resp = self.client.put('/api/v2/keyvalue/3/', {'system_id':'2', 'value':'1.1.1asdfasdf.1','key':'nic.0.ipv4_address.0'})
        self.assertEqual(resp.status_code, 401)

    def test_keyvalue_set_valid_ip(self):
        resp = self.client.put('/api/v2/keyvalue/3/', {'system_id':'2', 'value':'10.99.32.1','key':'nic.0.ipv4_address.0'})
        self.assertEqual(resp.status_code, 200)

    def test_keyvalue_set_invalid_mac_address(self):
        resp = self.client.put('/api/v2/keyvalue/3/', {'system_id':'2', 'value':'asdfsadfsadf','key':'nic.0.mac_address.0'})
        self.assertEqual(resp.status_code, 401)

    def test_keyvalue_set_valid_mac_address(self):
        resp = self.client.put('/api/v2/keyvalue/3/', {'system_id':'2', 'value':'00:00:00:00:00:00','key':'nic.0.mac_address.0'})
        self.assertEqual(resp.status_code, 200)

    def test_keyvalue_set_invalid_is_dhcp_scope(self):
        resp = self.client.put('/api/v2/keyvalue/3/', {'system_id':'2', 'value':'true','key':'is_dhcp_scope'})
        self.assertEqual(resp.status_code, 401)
    ...
    def test_keyvalue_set_invalid_dhcp_scope_start(self):
        resp = self.client.put('/api/v2/keyvalue/3/', {'system_id':'2', 'value':'1.1.1asdfasdf.1','key':'dhcp.scope.start'})
        self.assertEqual(resp.status_code, 401)

    def test_keyvalue_set_valid_dhcp_scope_start(self):
        resp = self.client.put('/api/v2/keyvalue/3/', {'system_id':'2', 'value':'10.99.32.1','key':'dhcp.scope.start'})
        self.assertEqual(resp.status_code, 200)

    def test_keyvalue_set_invalid_dhcp_scope_end(self):
        resp = self.client.put('/api/v2/keyvalue/3/', {'system_id':'2', 'value':'1.1.1asdfasdf.1','key':'dhcp.scope.end'})
        self.assertEqual(resp.status_code, 401)

    def test_keyvalue_set_valid_dhcp_scope_end(self):
        resp = self.client.put('/api/v2/keyvalue/3/', {'system_id':'2', 'value':'10.99.32.1','key':'dhcp.scope.end'})
        self.assertEqual(resp.status_code, 200)


    def test_keyvalue_set_invalid_dhcp_pool_start(self):
        resp = self.client.put('/api/v2/keyvalue/3/', {'system_id':'2', 'value':'1.1.1asdfasdf.1','key':'dhcp.pool.start'})
        self.assertEqual(resp.status_code, 401)

    def test_keyvalue_set_valid_dhcp_pool_start(self):
        resp = self.client.put('/api/v2/keyvalue/3/', {'system_id':'2', 'value':'10.99.32.1','key':'dhcp.pool.start'})
        self.assertEqual(resp.status_code, 200)

    def test_keyvalue_set_invalid_dhcp_pool_end(self):
        resp = self.client.put('/api/v2/keyvalue/3/', {'system_id':'2', 'value':'1.1.1asdfasdf.1','key':'dhcp.pool.end'})
        self.assertEqual(resp.status_code, 401)

    def test_keyvalue_set_valid_dhcp_pool_end(self):
        resp = self.client.put('/api/v2/keyvalue/3/', {'system_id':'2', 'value':'10.99.32.1','key':'dhcp.pool.end'})
        self.assertEqual(resp.status_code, 200)
    ...
    def test_keyvalue_set_invalid_dhcp_ntp_server(self):
        resp = self.client.put('/api/v2/keyvalue/3/', {'system_id':'2', 'value':'1.1.1asdfasdf.1','key':'dhcp.option.ntp_server.0'})
        self.assertEqual(resp.status_code, 401)

    def test_keyvalue_set_valid_dhcp_ntp_server(self):
        resp = self.client.put('/api/v2/keyvalue/3/', {'system_id':'2', 'value':'10.99.32.1','key':'dhcp.option.ntp_server.0'})
        self.assertEqual(resp.status_code, 200)
