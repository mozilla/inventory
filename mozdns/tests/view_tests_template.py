...
class GenericViewTests(object):
    ...
    def setUp(self):
        ...
        server = "random"
        self.test_obj, create = Nameserver.objects.get_or_create(server=server,
                                                        domain= self.domain)
        while not create:
            server = "a"+server
    ...
    def build_get_create_in_domain(self):
        """
        ex: url(r'^/mozdns/domain/(?P<domain>[\w-]+)/create$',
    ...
    def build_get_object_update(self):
        """
        ex: url(r'^/mozdns/domain/(?P<pk>[\w-]+)/update$',
        DomainUpdateView.as_view()),
        """
    ...
    def build_get_object_details(self):
        """
        ex: url(r'^/mozdns/domain/(?P<pk>[\w-]+)/$',
        DomainDetailView.as_view()),
        """
        def test_get_object_details(self):
            resp = self.client.get(
                MOZDNS_BASE_URL + "/%s/%s/" % (self.url_slug,
    ...
    def build_get_object_delete(self):
        """
        ex: url(r'^/mozdns/domain/(?P<pk>[\w-]+)/delete$',
